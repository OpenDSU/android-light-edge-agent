...
internal class RequestInspectorJavaScriptInterface(webView: WebView) {
    ...
    fun findRecordedRequestForUrl(nonce: Int?, url: String): RecordedRequest? {
        if (nonce != null) {
            val nonceResult = recordedRequests.find { recordedRequest -> recordedRequest.nonce == nonce }
            if (nonceResult != null) {
                return nonceResult
            }
        }
    ...
    fun recordFormSubmission(
        ...
        recordedRequests.add(
                RecordedRequest(WebViewRequestType.FORM, url, method, body, headerMap, trace, enctype, nonce)
        )
    }

    @JavascriptInterface
    fun recordXhr(nonce: Int, url: String, method: String, body: String, headers: String, trace: String) {
        Log.i(LOG_TAG, "Recorded XHR from JavaScript")
        val headerMap = getHeadersAsMap(headers)
        recordedRequests.add(
                RecordedRequest(WebViewRequestType.XML_HTTP, url, method, body, headerMap, trace, null, nonce)
        )
    }

    @JavascriptInterface
    fun recordFetch(nonce: Int, url: String, method: String, body: String, headers: String, trace: String) {
        Log.i(LOG_TAG, "Recorded fetch from JavaScript")
        val headerMap = getHeadersAsMap(headers)
        recordedRequests.add(
                RecordedRequest(WebViewRequestType.FETCH, url, method, body, headerMap, trace, null, nonce)
        )
    }
...
function recordFormSubmission(form) {
    const nonce = ++window.requestNonce;
    var jsonArr = [];
...
window.fetch = function () {
    ...
    let err = new Error();
    $INTERFACE_NAME.recordFetch(nonce, fullUrl, method, body, headers, err.stack);
